// Mac CD-ROM: the AppleCD playhead (see mac_cdrom_play.h). Each block names
// the rtl/cd_audio.sv state it transcribes; keep them in step.
...
// the CDB's 32-bit LBA form, clamped to the 20-bit disc space (cdb_lba)
...
static uint32_t msf2lba_bcd(uint8_t m, uint8_t s, uint8_t f)   // msf2lba
{
	return L20((uint32_t)mac_cd_bcd2bin(m) * 4500u + (uint32_t)mac_cd_bcd2bin(s) * 75u + mac_cd_bcd2bin(f));
}

static uint32_t msf2lba_std(uint8_t m, uint8_t s, uint8_t f)   // msf2lba_std: binary, +150 space
{
...
{
	// cd_audio.sv flush: the frame halves are dropped and fetch_lba
	// re-syncs to cur_lba
	p->fetch = p->cur;
	p->flush_gen++;
}
...
void mac_cd_play_init(mac_cd_play *p)
{
	...
	p->ports[0] = 0x01; p->ports[1] = 0xFF;   // ncr53c96.sv reset: port 0 = left, full
	p->ports[2] = 0x02; p->ports[3] = 0xFF;   //                    port 1 = right, full
}

void mac_cd_play_set_toc(mac_cd_play *p, const mac_cd_toc *t)
{
	// M_IDLE on img_mounted: playback stops; cur/stop survive (they only
...
{
	// (read_stb || eject_stb || bus_rst || !mounted) && pstate != ST_IDLE
...
// MODE SELECT(6) list, the ncr53c96.sv msel_st parse: a block descriptor
// must fit the list and ask for 2048-byte blocks, and page $0E's output
// ports are taken from page bytes 8..11 (the list beyond its stated length
// is whatever the core's buffer held, exactly as the RTL reads it).
static void mode_select(mac_cd_play *p, const uint8_t *list, int list_len)
{
	if (list_len < 4) return;                 // the RTL only parses a list with a header
	unsigned bd = list[3];
	if (bd != 0)
	{
		if (bd + 4 > (unsigned)list_len) return;             // 05/26: refused
		if (list[10] != 0x08 || list[11] != 0x00) return;    // anything but 2048: refused
	}
	unsigned base = bd ? 12 : 4;
	if (base + 2 > (unsigned)list_len) return;               // GOOD, nothing to take
...
// M_CTRK_RD: track-mode addresses, start(k) and start(k+1) or the lead-out
...
// M_APPLY for the range-play family (0x47/0x48/0x45/0xA5)
static void apply_range(mac_cd_play *p, uint32_t addr, uint32_t next)
{
	if (addr == next)
	{
		// zero-length play = SEEK-ONLY per SCSI-2: the position moves, the
		// state holds (the player's Next/Prev/Stop park the pickup this way)
		p->cur = addr;
		flush(p);
	}
	else
...
void mac_cd_play_command(mac_cd_play *p, const uint8_t *c, const uint8_t *list, int list_len)
{
	...
	// M_CMD: any transport command other than the scan itself ends a scan
	if (op != 0xCD) p->scan = 0;

	switch (op)
	{
	case 0xCA:                                             // AUDIO PAUSE
		if (c[1] == 0x10) { if (p->state == MAC_CD_ST_PLAY) p->state = MAC_CD_ST_PAUSE; }
		else if (p->state == MAC_CD_ST_PAUSE) p->state = MAC_CD_ST_PLAY;
		break;

	case 0xCD:                                             // AUDIO SCAN (FF/RW), standard decode
	...
	case 0xC8: case 0xC9: case 0xCB:                       // vendor SEARCH / PLAY / STOP
		if (form == 1)
		{
		...
		else if (form == 2)
		{
			...
			if (t == 0)
			{
				if (op != 0xCB) p->state = MAC_CD_ST_IDLE;   // track 0: stop
				return;
			}
		...
		else if (op == 0xC9)
		{
			if (c[1] & 0x10)
			{
				// the address is the END; start stays at the current position
				p->stop = addr;
	...
	case 0x45: case 0xA5:                                  // PLAY AUDIO(10)/(12), LBA form
	{
	...
	case 0x4E: case 0x01: case 0x0B: case 0x2B:            // STOP PLAY / REZERO / SEEK
...
// FETCH FSM condition + the frame_done advance (+-8 while scanning)
int mac_cd_play_frame(mac_cd_play *p, uint32_t *lba)
{
	...
	*lba = p->fetch;
	...
	{
		p->cur = (p->cur > 8) ? p->cur - 8 : 0;      // rewind: clamp at the disc start
	}
...
// M_REF_SCAN / M_REF_DIVA / M_REF_DIVR: the last track whose start is at or
// before the playhead; absolute and relative M/S/F, binary, no +150
...
// rtl/cd_vol_lut.vh: gain = (vol/255)^5 in Q15; 255 is exact unity, 0 mute
